...
const validateCred = arr =>{
    ...
    luhn = luhn.reverse();
    sum = luhn.reduce((accumulator, currentValue)=>accumulator + currentValue
    );
    if(sum % 10 == 0){
      return true
    }else{
...
const findInvalidCards = listOfArr =>{
  const invalidCards = []
  for(const no of listOfArr){
    if(validateCred(no) == false){
      invalidCards.push(no);
    }
  }
  return invalidCards;
}
